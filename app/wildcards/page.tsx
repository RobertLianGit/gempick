  ...
  return (
    <div className="page-shell flow-page">
      <header className="flow-header compact-header">
        <p className="eyebrow">逐首挑选</p>
        <h1>{selected.length < MIN_TRACKS ? "还想带上哪几首？" : "想带上的歌都在这里了吗？"}</h1>
        <p>{selected.length < MIN_TRACKS ? `目前有 ${selected.length} 首歌曲，还需要留下 ${MIN_TRACKS - selected.length} 首，才能开始这段选择。` : "逐首确认后就可以开始相遇；不同语言、现场或重录版本会作为不同录音版本出现。"}</p>
      </header>
      <section className="top-action-panel wildcard-action-panel">
        <div className="top-action-status">
          ...
          <div>
            ...
            <span>{selected.length < MIN_TRACKS ? `还差 ${MIN_TRACKS - selected.length} 首，才能开始这段旅程` : `接下来还要做 ${selected.length - 1} 次选择`}</span>
      ...
      <div className="reference-heading">
        <div><p className="eyebrow">候选歌曲参考</p><h2>加入这首，或者先放下</h2></div>
